This directory contains cmd files for running demos
showing how to use the CMS content type EncryptedData
for password-based encrypting digital documents.

The required libraries (iaik_jce.jar, iaik_cms.jar, 
iaik_cms_demo.jar) have to be located in the 
../../../lib directory. If you are using a JDK
version > 1.3, you will need the signed version of 
iaik_jce.jar located in the ../../../lib/signed directory.



EncryptedDataDemo:
==================

Shows how to use the IAIK-CMS EncryptedData and
EncryptedDataStream implementations.

Usage:

Start the demo by running

   > runEncryptedDataDemo.bat 


EncryptedDataOutputStreamDemo:
==============================

Shows how to use the IAIK-CMS EncryptedDataOutputStream
implementation.

Usage:

Start the demo by running

   > runEncryptedDataOutputStreamDemo.bat 






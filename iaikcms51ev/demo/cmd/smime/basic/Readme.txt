This directory contains cmd files for running 
...
Start the demo by running the batch file and specifying
mail host, sender name, sender and recipient mail address:

   > runSMimeSendDemo.bat [-H host] [-S sender name] [-F (From) sender address] [-T (To) recipient address]

e.g.:


   > runSMimeSendDemo.bat -H mailhost -S "John SMime" -F smimetest@iaik.tugraz.at -T smimetest@iaik.tugraz.at
...
account:
   ...
   > runSMimeShowDemo.bat [-T protocol] [-H host] [-U user] [-P password]

e.g.:


   > runSMimeShowDemo.bat -T pop3 -H mailhost -U smimetest -P password
